...
def _apply_config(
    backend: RenderBackend, config: Th07Config, music: BgmPlayer | None = None
) -> None:
    ...
    if music is not None and music.mode != config.music_mode:
        # cfg.music_mode 调档: StopAudio → 重载标题 BGM(MainMenu.cpp:617-637;
        # Option 是主菜单的一个态, 重载的就是标题曲)
        music.set_mode(config.music_mode)
        music.stop()
        music.play(TITLE_BGM)
...
def run_app(
    ...
    *,
    ...
    music = BgmPlayer(
        archive,
        (
            Path(assembly.resources.data_path).with_name(assembly.resources.bgm_file)
            if assembly.resources.bgm_file
            else None
        ),
        mode=config.music_mode,
    )
    _apply_config(backend, config, music)
    ...
    def make_option(title: TitleScene) -> OptionScene:
        return OptionScene(
            ...
            on_config_changed=lambda cfg: _apply_config(backend, cfg, music),
        )
    ...
    try:
        run_scenes(
            make_title(), backend, title=assembly.title, scale=scale, music=music
        )
    finally:
